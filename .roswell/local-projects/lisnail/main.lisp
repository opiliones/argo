...
(defun read-quote (stream c0)
  (loop with str = (make-array '(0) :element-type 'character
			       :adjustable t :fill-pointer t)
     for c = (read-char stream t nil t)
     until (and (char= c c0) (not (char= (peek-char nil stream t nil t) c0)))
     do (vector-push-extend
         (if (char= c c0) (read-char stream t nil t) c)
         str)
     finally (return str)))

(defun read-single-character-symbol (stream char)
  (declare (ignore stream))
  (intern (coerce `(,char) 'string)))

(defun read-multi-character-symbol (stream char)
  (let ((next (peek-char nil stream t nil t)))
    (intern (concatenate 'string
              (coerce `(,char ) 'string)
              (when (or (char= next char) 
                        (not (terminating-char-p next)))
                (string (read stream)))))))

(defun install-sh-reader (readtable)
  ...
  (set-macro-character #\] #'read-single-character-symbol nil readtable)
  (set-macro-character #\^ #'read-single-character-symbol nil readtable)
  (set-macro-character #\& #'read-multi-character-symbol nil readtable)
  (set-macro-character #\| #'read-multi-character-symbol nil readtable)
  (set-macro-character #\$ #'read-multi-character-symbol nil readtable)
  (set-macro-character #\! #'read-multi-character-symbol nil readtable)
  (set-macro-character #\. #'read-multi-character-symbol t readtable)
  (set-macro-character #\, #'read-multi-character-symbol t readtable)
  (set-macro-character #\: #'read-multi-character-symbol t readtable)
  (set-macro-character #\? #'read-multi-character-symbol t readtable)
  (set-macro-character #\~ #'read-multi-character-symbol t readtable)
...
(set-dispatch-macro-character #\# #\{
  (lambda (stream char1 char2)
    ...
    (let* ((*previous-syntax* *readtable*)
      ...
      (install-sh-reader *readtable*)
      (let* ((command-line (read-delimited-list #\} stream t))
             (command (car command-line))
             (args (rest command-line)))
        (if (fboundp command)
          (list command `',args)
          (list 'sb-ext:run-program (princ-to-string command)
