...
(cffi:defcfun "pipe" :int (arr :pointer))
(defun unix-pipe ()
  (cffi:with-foreign-objects ((arr :int 2))
    (pipe arr)
    (values (cffi:mem-aref arr :int 0)
            (cffi:mem-aref arr :int 1))))

(cffi:defcfun ("close" unix-close) :int (fd :int))

(defun make-pipe ()
  #+allegro (excl:make-pipe-stream)
  #+clisp (multiple-value-bind (r w) (unix-pipe)
            (print r)
            (print w)
            (values (ext:make-stream r :direction :input)
                    (ext:make-stream w :direction :output)
                    r w))
